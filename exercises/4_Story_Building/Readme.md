...
&#x2757;&#x2757; Please note that there is only one tab in the sample story for this exercise. The screenshots show two tabs. You only need the one you see in the story in SAC. &#x2757;&#x2757;
...
3. Let's also change the underlying data model of the table from the sample content to our own model which we previously created. To do so we open the Designer side panel, click on change primary model and select the planning model we created in the TechEd folder in **My Files**.
...
10. We now delete the 2022 Actual data from the table (as the zero values in 2022 are just a data quality problem). For this we collapse the hierarchy nodes in the table. Then we select the values in the column 2022 and hit the delete key. The final result should look like below. 
